      ...
      py.on("close", (code) => {
        ...
        if (code === 0 && outTrim) {
          try {
            ...
            console.error(
              "JSON parse error from python stdout:",
              outTrim,
              stderr,
              err,
            );
            res
              ...
              .json({
                ok: false,
                error: "Invalid JSON from Python model",
                stdout: outTrim,
                stderr,
              });
          }
          ...
          console.warn("Python script failed or produced no output", {
            code,
            stderr,
            stdout,
          });
...
function runJsFallback(payload: any) {
  ...
  const branchMul =
    { "สาขา A": 1.0, "สาขา B": 1.2, "สาขา C": 0.8 }[branch] || 1.0;
  ...
  const stock = {
    flour: Number(inputs.flour || 50000),
    butter: Number(inputs.butter || 15000),
    sugar: Number(inputs.sugar || 40000),
    eggs: Number(inputs.eggs || 300),
  };
  ...
  for (const { p } of scored) {
    ...
    const allocate = Math.max(
      0,
      Math.floor(Math.min(maxUnits, Math.max(10, capacity * 0.35))),
    );
    ...
    if (
      (forecast[p.name] || 0) > 0 &&
      expected_leftover / Math.max(1, forecast[p.name] || 0) > 0.3
    ) {
    ...
    plan.push({
      ...
      key: p.key,
      quantity: allocate,
      profitPerUnit: p.price - cost,
      expected_leftover,
      selling_price: p.price,
      product_cost: +cost.toFixed(2),
      gp_margin,
